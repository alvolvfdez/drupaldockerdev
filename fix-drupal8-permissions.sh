...
sudo find . -type d -exec chmod u=rwx,go=rx {} \;
echo -e "Changing permissions of all files inside "${path}" to "644"...n"
sudo find . -type f -exec chmod u=rw,go=r {} \;
...
sudo find . -name files -type d -exec find '{}' -type f \; | while read FILE; do sudo chmod ug=rw,o=r "$FILE"; done
echo "Changing permissions of all directories inside all "files" directories in "${path}/sites" to "775"..."
sudo find . -name files -type d -exec find '{}' -type d \; | while read DIR; do sudo chmod ug=rwx,o=rx "$DIR"; done
